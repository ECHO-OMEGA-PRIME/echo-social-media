...
function slog(level: 'info' | 'warn' | 'error', msg: string, data?: Record<string, unknown>) {
  const entry = { ts: new Date().toISOString(), level, worker: 'echo-social-media', version: '1.0.0', msg, ...data };
...
app.get('/status', async (c) => {
  try {
    ...
    console.error(JSON.stringify({ ts: new Date().toISOString(), level: 'error', service: 'echo-social-media', msg, path: c.req.path }));
